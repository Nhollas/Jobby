...
public class AuthenticatingUser_WithInvalidToken(JobbyHttpApiFactory factory)
{
    [Fact]
    public async Task WhenTokenHasExpired_ThenReturns401Unauthorized()
    {
        string expiredToken = JwtHelper.Generate(factory.UserId, expires: DateTime.UtcNow.AddDays(-1));
    ...
    public async Task WhenTokenHasInvalidSignature_ThenReturns401Unauthorized()
    {
        ...
        string tokenWithInvalidSecret = JwtHelper.Generate(factory.UserId, privatePemKey: invalidPem);
    ...
    public async Task WhenTokenHasInvalidIssuer_ThenReturns401Unauthorized()
    {
        ...
        string tokenWithInvalidIssuer = JwtHelper.Generate(factory.UserId, issuer: invalidIssuer);
